# image file
jpeg('out/drc_totals.jpg', res=300, height=6, width=6, units='in')

# plot function
source('code/functions/plotTotals.R')

# load data
d_random <- read.csv('out/drc/random/d.csv', check.names=F)
d_weighted <- read.csv('out/drc/weighted/d.csv', check.names=F)
d_weighted_naive <- read.csv('out/drc/weighted_naive/d.csv', check.names=F)
d_combined <- read.csv('out/drc/combined/d.csv', check.names=F)
